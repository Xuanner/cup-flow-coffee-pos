...
**完成证据：** `TC-S2-PASS-001` 至 `005` 通过。

#### TASK-S2-SEC-01-02 增加密码敏感信息防泄露测试

| 属性 | 内容 |
| --- | --- |
| 类型 / 状态 / 复杂度 | 后端安全测试 / Blocked / S |
| 主 Story | US-S2-SEC-01 |
| 依赖 | SEC-01-01 |

**工作内容：** 使用专用假凭证检查异常、日志、测试报告和快照；断言不存在明文密码或密码摘要。

**完成证据：** 植入假敏感值时检查可失败，正常执行无敏感值命中。
...
**完成证据：** `TC-S2-AUTH-001` 至 `005`、`009`、`010` 通过。

#### TASK-S2-AUTH-01-03 建立前端认证 API 与数据模型

| 属性 | 内容 |
| --- | --- |
| 类型 / 状态 / 复杂度 | 前端 / Blocked / M |
| 主 Story | US-S2-AUTH-01 |
...
**完成证据：** API 单元测试覆盖成功解析、凭证配置、错误映射和响应脱敏。
...
**完成证据：** `TC-S2-FE-AUTH-001` 至 `006` 通过，覆盖键盘操作和无障碍播报。

#### TASK-S2-AUTH-01-05 实现登录后的安全目标跳转

| 属性 | 内容 |
| --- | --- |
| 类型 / 状态 / 复杂度 | 前端路由、安全 / Blocked / M |
| 主 Story | US-S2-AUTH-01 |
| 支持后续 | US-S2-AUTHZ-01 |
| 依赖 | AUTH-01-04 |
...
**完成证据：** 安全返回地址测试与 `TC-S2-FE-ROUTE-005` 通过。
